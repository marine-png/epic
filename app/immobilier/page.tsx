...
const services = [
  {
    ...
    tag: "Entrepreneurs & Investisseurs actifs",
  },
  {
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-12">
            ...
            <div className="w-12 h-1 bg-[#C9601A]" />
          </div>
                ...
                <ul className="space-y-2">
                  {s.bullets.map((b) => (
                    <li key={b} className="flex items-start gap-2 text-sm text-gray-500">
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[#C9601A] flex-shrink-0" />
                      {b}
                    </li>
                  ))}
                </ul>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((step) => (
              <div key={step.num} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                <div className="w-10 h-10 rounded-full bg-[#C9601A] text-white flex items-center justify-center font-bold text-sm mb-4">
                  {step.num}
                </div>
                <h3 className="font-bold text-[#0f1e3c] mb-2 text-sm">{step.title}</h3>
                <p className="text-xs text-gray-500 leading-relaxed">{step.desc}</p>
                {step.withCTA && (
                  <Link href="/contact" className="inline-block mt-3 text-xs font-semibold text-[#C9601A] border border-[#C9601A] px-3 py-1 rounded hover:bg-[#C9601A] hover:text-white transition-colors">
                    Rendez-vous
                  </Link>
                )}
              </div>
            ...
            <div className="flex items-start gap-6">
              ...
              <div>
                ...
                <ul className="mt-4 space-y-1">
                  {["Expertise sur les locaux commerciaux et fonds de commerce", "Conseil en acquisition et investissement immobilier privé", "Suivi personnalisé et multilingue pour sécuriser vos projets"].map((item) => (
